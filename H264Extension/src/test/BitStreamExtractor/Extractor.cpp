...
Extractor::init( ExtractorParameter *pcExtractorParameter )
{
    ...
    if( !m_pcExtractorParameter->getAnalysisOnly() )
    {
        ...
        RNOKS( pcWriteBitstreamFile->init( m_pcExtractorParameter->getBaseFile() ) );
        m_pcWriteBitstream = (WriteBitstreamIf*)pcWriteBitstreamFile;

        WriteBitstreamToFile*  pcWriteEnhancement[pcExtractorParameter->getNumEnh()];
        m_pcWriteEnhancement = new WriteBitstreamIf* [pcExtractorParameter->getNumEnh()];
        for(UInt n = 0; n < pcExtractorParameter->getNumEnh(); n++)
        {
            RNOKS( WriteBitstreamToFile::create( pcWriteEnhancement[n] ) );
            RNOKS( pcWriteEnhancement[n]->init( m_pcExtractorParameter->getEnhFile()[n] ) );
            m_pcWriteEnhancement[n] = (WriteBitstreamIf*)(pcWriteEnhancement[n]);
        }
    }
    else
    {
        m_pcWriteBitstream = NULL;
        m_pcWriteEnhancement = NULL;
    }
...
Extractor::destroy()
{
    ...
    if( NULL != m_pcWriteEnhancement )
    {
        for(UInt n = 0; n < m_pcExtractorParameter->getNumEnh(); n++)
        {
            RNOK( m_pcWriteEnhancement[n]->uninit() );
            RNOK( m_pcWriteEnhancement[n]->destroy() );
        }
    }
...
Extractor::go()
{
    ...
    else if( m_pcExtractorParameter->getExtractionList().empty() ) // We always enter this condition
    {
...
Extractor::xExtractLayerLevel() // this function for extracting using "-sl, -l, -t, -f, -b" and its permittable combination
{
    ...
    UInt uiMaxTempLevel        = m_pcExtractorParameter->getLevel(); // this is specified as input (-t argument)
    UInt uiMaxFGSLayer         = m_pcExtractorParameter->getFGSLayer();
    UInt enhLayer              = m_pcExtractorParameter->getNumEnh();
    ...
    UInt maxTempLevel = m_cScalableStreamDescription.getNumOfScalableLayers() - 1;
    UInt baseLevel = m_pcExtractorParameter->getBaseLevel();
    UInt* enhLimits = m_pcExtractorParameter->getEnhLevel();
    UInt enhNo = m_pcExtractorParameter->getNumEnh();
    UInt countPackets[enhNo + 1];
    UInt uiPacketSize[enhNo + 1];
    for(UInt k = 0; k <= m_pcExtractorParameter->getNumEnh(); k++)
    {
        countPackets[k] = 0;
        uiPacketSize[k] = 0;
    }
    ...
    /////////////////////////////Define the Id assigner class
    NalIdAssign idAssign;
    idAssign.sepLayer = separate_layer;
    /////////////////////////////////////////////////////////
    ...
    while( ! bEOS )
    {
        ...
        //printf("Type: %d\n",cPacketDescription.NalUnitType);
        //prefix unit{{
        if(cPacketDescription.NalUnitType == 14)
        {
            if(pcBinData_prefix)
        ...
        if( bWriteBinData )
        {
            ...
            if( ! bApplyToNext  )
            {
                //prefix unit{{ But that's not the prefix unit!
                if(cPacketDescription.NalUnitType == 1 || cPacketDescription.NalUnitType == 5)
                {
                ...
                //prefix unit}}
                uiLayer          = cPacketDescription.Layer;
                uiTempLevel      = cPacketDescription.Level; // Determines the size of the base layer.
                uiFGSLayer      = cPacketDescription.FGSLayer;
            ...
            if(m_pcExtractorParameter->getKeepfExtraction())
            {
                if( uiWantedScalableLayer == MSYS_UINT_MAX )  //input: -t,-f
                {
                    ...
                    ////////////////////////////////The case of multiple enhancement layers///////////////////
                    //For now, we only assume temporal scalability
                    if(uiTempLevel <= baseLevel)
                        bKeepTemporal = 0; //Indicating a base layer NAL unit.
                    else if(uiTempLevel > uiMaxTempLevel || uiTempLevel > maxTempLevel)
                        bdiscard = true;
                    else
                    {   //The or statement is for the case that the -t argument is set too large!
                        if(uiMaxTempLevel - baseLevel < enhNo || maxTempLevel - baseLevel < enhNo)
                        {
                            printf("Error! There are less enhancement layers than requested.");
                            bKeepTemporal = 1; // Put them all into a single enhancement layer.
                        }
                        else if(uiMaxTempLevel - baseLevel == enhNo || maxTempLevel - baseLevel == enhNo)
                        {
                            bKeepTemporal = uiTempLevel - baseLevel; //Ignoring the requested enh layer separation.
                        }
                        else
                        {
                            for(UInt l = enhNo; l >= 1; l--)
                            {
                                if(enhLimits[l - 1] > maxTempLevel || enhLimits[l - 1] > uiMaxTempLevel)
                                {
                                    printf("Error! Crossed the maximum number of temporal layers.");
                                    bKeepTemporal = l - 1;
                                    break;
                                }
                                else if(uiTempLevel <= enhLimits[l - 1])
                                {
                                    bKeepTemporal = l;
                                }
                            }
            ...
            if(  eNalUnitType == NAL_UNIT_SPS )
            {
                ...
                if(bKeep)
                    ...
                    bKeepTemporal = 1; // Put it in the first enhancement layer.
            ...
            if(bKeepTemporal == 0 && !bdiscard)
            {
                //prefix unit{{
                if((cPacketDescription.NalUnitType == 1 || cPacketDescription.NalUnitType == 5) && pcBinData_prefix != NULL )
                {
                    keep++;
                    RNOK( idAssign.enterNalId(pcBinData_prefix));
                    RNOK( m_pcWriteBitstream->writePacket( &m_cBinDataStartCode ) );
                    RNOK( m_pcWriteBitstream->writePacket( pcBinData_prefix ) );
                    countPackets[bKeepTemporal]++;

                    uiPacketSize[bKeepTemporal] += 4 + pcBinData_prefix->size();
                    RNOK( m_pcReadBitstream->releasePacket( pcBinData_prefix ) );
                    pcBinData_prefix = NULL;
                    uiNumKept++;
                    uiNumInput++;
                ...
                keep++;
                RNOK( idAssign.enterNalId(pcBinData));
                RNOK( m_pcWriteBitstream->writePacket( &m_cBinDataStartCode ) );
                RNOK( m_pcWriteBitstream->writePacket( pcBinData ) );
                countPackets[bKeepTemporal]++;

                uiPacketSize[bKeepTemporal] += 4 + pcBinData->size();
            }
            else if(bKeepTemporal > 0 && !bdiscard)
            {
                if((cPacketDescription.NalUnitType == 1 || cPacketDescription.NalUnitType == 5) && pcBinData_prefix != NULL)
                {
                    discard++;
                    RNOK( idAssign.enterNalId(pcBinData_prefix));
                    RNOK( m_pcWriteEnhancement[bKeepTemporal - 1]->writePacket( &m_cBinDataStartCode ) );
                    RNOK( m_pcWriteEnhancement[bKeepTemporal - 1]->writePacket( pcBinData_prefix ) );
                    countPackets[bKeepTemporal]++;
                ...
                /////////////////////// Determine the enhancement layer substreams
                discard++;
                RNOK( idAssign.enterNalId(pcBinData));
                RNOK( m_pcWriteEnhancement[bKeepTemporal - 1]->writePacket(&m_cBinDataStartCode ) );
                RNOK( m_pcWriteEnhancement[bKeepTemporal - 1]->writePacket(pcBinData) );
                countPackets[bKeepTemporal]++;
        ...
        if( bWriteBinDataSEILysNotPreDepChange )
        {
            ...
            uiPacketSize[bKeepTemporal]  = 4 + pcBinDataSEILysNotPreDepChange->size();
    ...
    RNOK( m_pcH264AVCPacketAnalyzer->uninit() );
    for(UInt k = 0; k <= m_pcExtractorParameter->getNumEnh(); k++)
        printf("Number of packets for layer %d is equal to: %i. The size is: %d\n",k,countPackets[k],uiPacketSize[k]);
    printf("Latest NAL unit ID assigned: %d\n", idAssign.getNalId());
