...
: m_cInFile       () // name of inout file
, m_cBaseFile      () // name of output file
, m_cEnhFile       ()
, m_iResult       ( -10 )
, m_uiScalableLayer( MSYS_UINT_MAX )
, m_uiLayer       ( MSYS_UINT_MAX )   // This refers to the dependency layer
, m_uiLevel       ( MSYS_UINT_MAX )  // This refers to the temporal layer.
, m_uiFGSLayer    ( 1000 )
, m_dBitrate      ( MSYS_UINT_MAX )
, m_bAnalysisOnly ( true )
// HS: packet trace
, m_bTraceFile    ( false )    //wether we have a packet trace file
, m_bTraceExtract ( false )
, m_cTraceFile    ()    // name of packet trace file
, m_cExtractTrace ()
...
#define EXIT(x,m) {if(x){printf("\n%s\n",m);RNOKS(xPrintUsage(argv))}}
  ...
  //generates a packet trace file from the given stream.
  if( argc > 3 && equal( "-pt", argv[1] ) ) // HS: packet trace
  {
    m_cTraceFile  = argv[2]; //Indicating the name of the trace file
    m_bTraceFile  = true;
    argv         += 2;
    argc         -= 2;
  }
  ...
  m_cInFile       = argv[1];  // The name of the input file
  ROTRS( m_bAnalysisOnly, Err::m_nOK );
  m_cBaseFile      = argv[2]; // The name of the output base file
  argv += 2;
  argc -= 2;
  
    //===== set specification for the base layer
    if(equal(argv[1],"-bl"))
    {
    ...
    if(argc > 1)
    {
        if(equal(argv[1],"-n"))
        {
            ...
            enhLevel = new UInt[numEnh];

            EXIT(argc < numEnh + 2,"Not enough Enhancement layers specfied");
            argv += 2;
            argc -= 2;

            for(UInt n = 0; n < 2*numEnh; n+=2)
            {
                m_cEnhFile[n/2] = argv[n + 1]; // The name of the output file for Enhancement layers
                enhLevel[n/2] = atoi(argv[n + 2]);
            }

            argc -= 2 * numEnh;
            
            m_uiLevel = enhLevel[numEnh - 1];
