...
{
public:  //includes a point class, QLExtractionMode enumeration
  class Point
  {
  public:
...
public: // (why public used again?) includes constructor, virtual function, bunch of functions
  ExtractorParameter          ();
  ...
  Void                  setBaseFileName      (std::string strName){ m_cBaseFile = strName;    }
  ...
  UInt getNumEnh () const {return numEnh;}
  ...
  UInt                  getBaseLevel()                    const{return baseLevel;}
  Void                  setBaseLevel(UInt newBase)        {baseLevel = newBase;}
  
  UInt*                 getEnhLevel()                     const{return enhLevel;}
  Void                  setEnhLevel(UInt* newEnh)         {enhLevel = newEnh;}
    ///Support for layer separation
