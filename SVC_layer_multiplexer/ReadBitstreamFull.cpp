...
ErrVal ReadBitstreamFull::extractPacket( BinData*& rpcBinData, bool& rbEOS )
{
    ...
    rpcBinData->set( new unsigned char[dwLength], dwLength );
...
ErrVal ReadBitstreamFull::init( const std::string& rcFileName )
{
    ...
    if ( (0) != m_cFile.open( rcFileName, LargeFile::OM_READONLY ) )
    {
    ...
    unsigned char  aucBuffer[0x4];
    unsigned int uiBytesRead;
    RNOK( m_cFile.read( aucBuffer, 4, uiBytesRead ) );
    
    unsigned int uiStartPos = (aucBuffer[0] == 0 && aucBuffer[1] == 0 && aucBuffer[2] == 1) ? 1 : 0;
